...
IMG ?= ghcr.io/openeverest/provider-cassandra-dev:latest
...
K3D_CLUSTER_NAME ?= provider-cassandra-test
...
helm-install: helm-deps ## Install the provider using Helm.
	helm install provider-cassandra $(CHART_DIR) --create-namespace

.PHONY: helm-upgrade
helm-upgrade: helm-deps ## Upgrade the provider using Helm.
	helm upgrade provider-cassandra $(CHART_DIR)

.PHONY: helm-uninstall
helm-uninstall: ## Uninstall the provider using Helm.
	helm uninstall provider-cassandra

.PHONY: helm-template
helm-template: helm-deps ## Render Helm chart templates locally (dry-run).
	helm template provider-cassandra $(CHART_DIR)
